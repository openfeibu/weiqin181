...
define('CLOUD_GATEWAY_URL', $_SERVER['HTTP_HOST'].'/gateway.php');
...
function cloud_build() {
    ...
    $dat = cloud_request($_SERVER['HTTP_HOST'].'/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
    ...
    $dat = cloud_request($_SERVER['HTTP_HOST'].'/gateway.php', $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_sms_send($mobile, $content, $postdata = array(), $custom_sign = '') {
    ...
    $response = cloud_request($_SERVER['HTTP_HOST'].'/gateway.php', $pars);
    if (is_error($response)) {
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request($_SERVER['HTTP_HOST'].'/gateway.php', $pars);
...
function cloud_auth_url($forward, $data = array()) {
    ...
    $auth_url = $_SERVER['HTTP_HOST'].'/gateway.php?__auth=' . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request($_SERVER['HTTP_HOST'].'/gateway.php', $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
