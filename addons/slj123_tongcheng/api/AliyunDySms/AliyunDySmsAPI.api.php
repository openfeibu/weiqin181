...
class AliyunDySmsAPI
{
     ...
     */
    public static function getAcsClient($accessKeyId, $accessKeySecret, $region = 'cn-hangzhou', $endPointName = 'cn-hangzhou') {
     ...
     * 发送短信
     *
     * @param string $setPhoneNumbers
     * @param string $setSignName
     * @param string $setTemplateCode
     * @param string $accessKeyId
     * @param string $accessKeySecret
     * @param array  $setTemplateParam
     * @param bool   $TLS
     * @param string $setOutId
     * @param string $setSmsUpExtendCode
     * @param string $region
     * @param string $endPointName
     * @return stdClass
     */
    public static function sendSms(
        ...
        $request->setTemplateParam(json_encode($setTemplateParam, JSON_UNESCAPED_UNICODE));

        // 可选，设置流水号
        $setOutId && $request->setOutId($setOutId);

        // 选填，上行短信扩展码（扩展码字段控制在7位或以下，无特殊需求用户请忽略此字段）
        $setSmsUpExtendCode && $request->setSmsUpExtendCode($setSmsUpExtendCode);

        // 发起访问请求
        $acsResponse = static::getAcsClient($accessKeyId, $accessKeySecret, $region, $endPointName)->getAcsResponse($request);
